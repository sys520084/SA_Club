...
<%j=0
  do while not rs.EOF and j<rs.PageSize
	...
	<TABLE cellpadding="4" cellspacing="1" width="95%" align="center">
	  <TR> 
		<TD bgcolor="#FFFFFF"> <table width="100%">
			<tr> 
			  ...
			  　　　<%=j&"&nbsp;<img src='../img/"&rs("face")&".gif'>"%> <a href="../membership/display.asp?name=<%=author%>" onclick="return js_callpages(this.href)"><%=rs("author")%></a> 于 <font color="green"><small><%=rs("ondate")&" "&rs("ontime")%></small></font> 
	  ...
	  <tr>
		<td> <div align="right">
		<a href="viewtitle.asp?author=<%=author%>" style="border: 1px solid #999999;padding: 2px;"><%=author%>的blog</a>&nbsp;
		 <a href="new.asp?id=<%=rs("id")%>&ThisTail=<%=MyUrlEncode(ThisTail&"&page="&page)%>&UrlTail=<%=UrlTail%>"  target="_blank" style="border: 1px solid #999999;padding: 2px;">'回复此文</a> 
			<%
			s=rsCount mod pgSz
			t=j mod pgSz
...
<%rs.MoveNext
  ...
  rs.Close
  set rs=nothing%>
	<form name="frmpage1" method="post" action="viewarticle.asp?<%=ThisTail%>&UrlTail=<%=UrlTail%>">
	  <input type="hidden" name=id value=<%=id%>>
	  <table width="98%" border="0" cellpadding="4">
		<tr align="center">
		 ...
		 <td width="10%">
		  ...
		  <%if page<pageCount then%></a><%end if%></td>
		  <td width="15%">跳转到第 
			<select name="page" onchange="if(this.value!='')frmpage1.submit();">
			<%for i=1 to pageCount%>
			   <option value=<%=i%> <%if page=i then%> selected <%end if%>><%=i%></option>
			<%next%>   
			</select>
